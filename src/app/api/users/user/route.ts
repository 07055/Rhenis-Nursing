// frontend\src\app\api\users\user\route.ts
import { NextRequest, NextResponse } from "next/server";
import { getBackendBaseUrl } from "@/lib/api/api/api";

export interface DynamicSystemUserResponse<T = unknown> {
  message?: string;
  error?: string;
...
// Base Route Logic 📌
const TARGET_NAME = "user";
const BASE_ROUTE = `/api/castoline/system/${TARGET_NAME}`;
...
async function forwardToBackend(
  ...
  try {
    ...
    console.log(`🌿 [roleRoute] Forwarding ${method} to:`, url);
   ...
   //  EXTRACT SESSION HEADERS FIRST
    ...
    console.log("🔐 Forwarded Session Headers :", {
      "x-casto-line": castoLine ?? "Void",
      "x-casto-badge": castoBadge ?? "Void",
    });
    ...
    const responseBody: DynamicSystemUserResponse = await backendResponse.json().catch(() => ({
    ...
    const errorMsg =
      err instanceof Error ? err.message : `Unknown ${method} error`;
...
export async function GET(req: NextRequest) {
  ...
  if (fetchtype === "current") {
      return forwardToBackend(req, "GET", `${BASE_ROUTE}/list/current`);
  }
