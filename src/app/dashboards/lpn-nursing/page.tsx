...
import LpnNursingListCard from "./includes/components/LpnNursingListCard";

const SUBJECT_CARDS = [
  { title: "HESI EXAMS (LPN)" },
  { title: "ATI EXAMS (LPN)" },
  { title: "EXIT ATI EXAMS-LPN" },
  { title: "EXIT HESI EXAMS-LPN" },
  { title: "EXAMPLIFY PRACTICE (PN)" },
  { title: "KAPLAN EXAMS" },
];

export default function LpnNursingHomePage() {
  ...
  return (
    ...
    >
      <div className="p-2 md:p-3 lg:p-3 space-y-3 lg:space-y-3 w-full max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-3 lg:items-stretch">
          {/* Greeting Card */}
          ...
          {/* LPN Nursing Subjects — order-2 on mobile, spans full width on desktop */}
          <div className="order-2 sm:col-span-2 lg:order-last lg:col-span-3">
            ...
            <div className="grid grid-cols-1 min-[360px]:grid-cols-2 sm:grid-cols-3 gap-4">
              {SUBJECT_CARDS.map((subject) => {
                return (
                  <div
                    key={subject.title}
                    className="relative overflow-hidden rounded-2xl bg-white shadow-md p-4 md:p-6 flex flex-col items-center text-center"
                  >
                    <h3 className="text-sm md:text-lg font-bold uppercase text-gray-900 tracking-wide leading-tight">
                      {subject.title}
                    </h3>
                    <Link
                      href="/register"
                      className="mt-3 md:mt-4 w-full inline-flex items-center justify-center py-2 md:py-3 rounded-lg text-xs md:text-sm font-bold text-white bg-[#14b8a6] hover:bg-[#0d9488] transition-colors duration-200"
                    >
                      Start Now
                    </Link>
                  </div>
          ...
          {/* LPN Nursing List Card */}
          <div className="shrink-0 order-3 sm:order-last lg:order-2">
            <LpnNursingListCard />
          </div>
        ...
        {activeExam && (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              ...
              <Link
                href={`/web/lpn-nursing/vista/exam/generic?mode=Exam&identifier=${activeExam.guidId ?? activeExam.id}`}
