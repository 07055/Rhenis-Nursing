...
export default function UpperStickyNavBar() {
    ...
    const isNewAttemptNav = examSession?.examActions?.[0]?.status === "Completed";
    const attemptModeKey = examSession?.exam?.guidId ? `exam_attempt_mode_${examSession.exam.guidId}` : null;
    const attemptMode = attemptModeKey ? sessionStorage.getItem(attemptModeKey) : null;
    const isFreshAttempt = isNewAttemptNav && attemptMode !== "prior";

    const answeredCount = allQuestions.filter((q) => {
        const ctxAnswer = getAnswer(q.id);
        if (ctxAnswer != null) return true;
        if (!q.savedUserAnswer) return false;
        // If fresh attempt: only count answers from current attempt number
        if (isFreshAttempt) return q.savedUserAnswer.attemptNumber === currentAttemptNumber;
        // Otherwise count any saved answer
        return true;
    }).length;
    ...
    const isNewAttemptTimer = examSession?.examActions?.[0]?.status === "Completed";
    const attemptModeKeyTimer = examSession?.exam?.guidId ? `exam_attempt_mode_${examSession.exam.guidId}` : null;
    const attemptModeTimer = attemptModeKeyTimer ? sessionStorage.getItem(attemptModeKeyTimer) : null;
    const shouldResetTimer = isNewAttemptTimer && attemptModeTimer !== "prior";

    useEffect(() => {
        if (totalSeconds <= 0) return;
        // Fresh new attempt → always start from full duration
        const startFrom = shouldResetTimer ? totalSeconds : (backendResidualSeconds ?? totalSeconds);
        setSecondsLeft(startFrom);
        setTimeWarning(false);
        setTimeUp(false);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [totalSeconds, shouldResetTimer]);
