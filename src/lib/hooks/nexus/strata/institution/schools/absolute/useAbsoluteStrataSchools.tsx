...
export interface StrataItem {
  ...
  // Parent Institution info
  institutionId?: number;
  institutionGuidId?: string;
  institutionName?: string;

  departmentsCount?: number;
}

export const useAbsoluteStrataSchools = ({
  page = 1,
  perPage = 20,
  sortColumn = null,
  sortDirection = "asc",
  ...
  useEffect(() => {
    const fetchSchools = async () => {
      try {
        console.log("🚀 [useAbsoluteStrataSchools] Fetching Schools ... 🪝");
        ...
        console.error(
          "❌ [useAbsoluteStrataSchools] Failed to fetch Schools:",
          err
        );
      }
    };
    ...
    fetchSchools();
  }, [schoolSearch, page, perPage, sortColumn, sortDirection]);

  // Pass Schools when search changes [ Frontend is just to display !]
  useEffect(() => {
